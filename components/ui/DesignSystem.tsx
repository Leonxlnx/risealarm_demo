        ...
        <div className={`overflow-hidden ${className}`}>
            <div className="flex flex-wrap gap-x-[0.25em] gap-y-0">
                {words.map((word, i) => (
                    <span
                        key={i}
                        className="inline-block origin-top-left opacity-0 animate-[word-up_0.6s_cubic-bezier(0.25,1,0.5,1)_forwards]"
...
export const Reveal = ({ children, className = "", delay = 0, mode = 'blur' }: any) => {
    ...
    // FAST DURATIONS - 0.6s for clean snappy feel
    const styles = {
        blur: {
            opacity: isVisible ? 1 : 0,
            transform: isVisible ? 'scale(1) translateY(0)' : 'scale(0.95) translateY(30px)',
            filter: isVisible ? 'blur(0)' : 'blur(12px)',
            transition: `all 0.6s cubic-bezier(0.25, 1, 0.5, 1) ${delay}ms`
        },
        slide: {
            opacity: isVisible ? 1 : 0,
            transform: isVisible ? 'translateY(0)' : 'translateY(50px)',
            transition: `all 0.5s cubic-bezier(0.25, 1, 0.5, 1) ${delay}ms`
        },
        mask: {}
    };
        ...
        return (
            <div ref={ref} className={`overflow-hidden ${className}`}>
                <div style={{
                    ...
                    transition: `transform 0.6s cubic-bezier(0.25, 1, 0.5, 1) ${delay}ms`
                }}>
                    {children}
                </div>
            </div>

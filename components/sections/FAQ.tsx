...
const FAQItem = ({ question, answer, isOpen, onClick }: any) => {
  ...
  return (
    <div 
      className={`border-b border-gray-200 py-8 md:py-10 transition-all duration-500 ${isOpen ? 'bg-transparent' : 'hover:bg-transparent'}`}
    >
      <h3>
        <button
          type="button"
          ...
          className="w-full flex justify-between items-start md:items-center gap-6 cursor-pointer group bg-transparent border-none p-0 appearance-none text-left"
        >
           <span className={`text-xl md:text-2xl font-bold transition-all duration-300 text-left leading-tight tracking-tight ${isOpen ? 'text-[#FF6B00]' : 'text-[#111] group-hover:text-gray-600'}`}>
              {question}
           </span>

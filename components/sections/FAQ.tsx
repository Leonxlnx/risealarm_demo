    ...
    <div className="border-b border-gray-200">
      <h3>
        <button
          ...
          className={`w-full py-8 md:py-10 cursor-pointer group transition-all duration-500 text-left flex justify-between items-start md:items-center gap-6 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#FF6B00] focus-visible:ring-offset-2 rounded-lg ${isOpen ? 'bg-transparent' : 'hover:bg-transparent'}`}
        >
          <span className={`text-xl md:text-2xl font-bold transition-all duration-300 leading-tight tracking-tight ${isOpen ? 'text-[#FF6B00]' : 'text-[#111] group-hover:text-gray-600'}`}>
            {question}
          </span>
          <span className={`shrink-0 w-10 h-10 rounded-full border-2 flex items-center justify-center transition-all duration-500 ${isOpen ? 'bg-[#FF6B00] border-[#FF6B00] rotate-180 text-white' : 'bg-transparent border-gray-200 text-gray-400 group-hover:border-[#111] group-hover:text-[#111]'}`}>
      ...
      <div
        ...
        role="region"
        aria-hidden={!isOpen}
        className={`grid transition-all duration-700 ease-[cubic-bezier(0.16,1,0.3,1)] ${isOpen ? 'grid-rows-[1fr] opacity-100 mb-8' : 'grid-rows-[0fr] opacity-0 mb-0'}`}
      >
        <div className="overflow-hidden">
          <p className="text-lg text-gray-500 leading-relaxed max-w-3xl text-left">
            {answer}
          </p>
        </div>
      </div>
